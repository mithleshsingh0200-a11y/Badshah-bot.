...
def get_goagames_period(mode):
    ...
    # Live Sync Logic: Period match karne ke liye
    p_count = (50000 + (total_seconds // 30)) if mode == "30s" else (10000 + (total_seconds // 60))
...
async def auto_loop(context, chat_id, mode):
    ...
    while user_active_modes.get(chat_id) == mode:
        try:
            ...
            text = f"👑 **BADSHAH KING VIP V33**\n\n🆔 **Period:** `{p}`\n📊 **Size:** {res}\n🎨 **Colour:** {color}\n🎯 **Jackpot Nos:** `{jacks[0]}, {jacks[1]}`\n⏳ **Status:** Syncing..."
            msg = await context.bot.send_message(chat_id=chat_id, text=text, parse_mode='Markdown')
            await asyncio.sleep(interval - 5)

            is_win = random.random() > 0.12 # Level 1-2 Win
            gift = "✅ **LEVEL 1 WIN 💸💸💸**" if is_win else "✅ **LEVEL 2 WIN 💰💰💰**"
            
            final_text = f"👑 **BADSHAH KING VIP V33**\n\n🆔 **Period:** `{p}`\n📊 **Result:** {res} {color}\n🎯 **Jackpot Nos:** `{jacks[0]}, {jacks[1]}`\n✨ **Status:** {gift}\n\n✅ Next prediction in 5s..."
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("🚀 30S MODE", callback_data='30s'), InlineKeyboardButton("🚀 60S MODE", callback_data='60s')]]
